...
import { vdiSessionDescriptionFactory } from './vdiSessionDescriptionHandler';

(window as any).jQuery = $;
(window as any).SIP = SIP;

$(function () {
	const bootstrap = require('bootstrap');
	const SDK = require('ringcentral');
	const WebPhone = require('ringcentral-web-phone') || (window as any).RingCentral.WebPhone;
	...
	function postLogin(server, appKey, appSecret, login, ext, password, ll) {
		...
		localStorage.setItem('webPhoneLogLevel', (logLevel as any) || '0');
	...
	function register(data) {
		...
		// console.error('VDI Config ', config);
		...
		webPhone = new WebPhone(data, {
			...
			logLevel: parseInt(logLevel as any, 10),
			...
			sessionDescriptionHandlerFactory: vdiSessionDescriptionFactory,
	...
	function onAccepted(session) {
		...
		$modal.find('.increase-volume').on('click', function () {
			session.userAgent.audioHelper.setVolume((session.userAgent.audioHelper.volume !== null ? session.userAgent.audioHelper.volume : 0.5) + 0.1);
		});
	...
	function makeCall(number, homeCountryId) {
		outboundCall = true;
		homeCountryId = homeCountryId || (extension && extension.regionalSettings && extension.regionalSettings.homeCountry && extension.regionalSettings.homeCountry.id) || null;
	...
	function makeCallForm() {
		...
		$username.html('<dl>' + '<dt>Contact</dt><dd>' + extension.contact.firstName + ' ' + extension.contact.lastName + '</dd>' + '<dt>Company</dt><dd>' + (extension.contact.company || '?') + '</dd>' + '<dt>Phone Number</dt><dd>' + username + '</dd>' + '</dl>');
